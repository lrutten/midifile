...
void Binasc::setMidiOn()
{
   setMidi(true);
}


void Binasc::setMidiOff()
{
   setMidi(false);
}
...
{   
   ...
   {   
      ...
      if (terminators.find(input[i]) == std::string::npos)
      {   
...
{   
   ...
   if (ch != 'M')
   {
      std::cerr << "Not a MIDI file M" << std::endl; return 0;
   }
   input.read((char*)&ch, 1);
   if (ch != 'T')
   {
      std::cerr << "Not a MIDI file T" << std::endl; return 0;
   }
   input.read((char*)&ch, 1);
   if (ch != 'h')
   {
      std::cerr << "Not a MIDI file h" << std::endl; return 0;
   }
   input.read((char*)&ch, 1);
   if (ch != 'd')
   {
      std::cerr << "Not a MIDI file d" << std::endl; return 0; 
   }
   tempout << "\"MThd\"";
   if (m_commentsQ)
   {   
   ...
   {   
      ...
      if (ch != 'M')
      {
         std::cerr << "Not a MIDI file M2" << std::endl; return 0;
      }
      input.read((char*)&ch, 1);
      if (ch != 'T')
      {
         std::cerr << "Not a MIDI file T2" << std::endl; return 0;
      }
      input.read((char*)&ch, 1);
      if (ch != 'r')
      {
         std::cerr << "Not a MIDI file r" << std::endl; return 0;
      }
      input.read((char*)&ch, 1);
      if (ch != 'k')
      {
         std::cerr << "Not a MIDI file k" << std::endl; return 0;
      }
      tempout << "\"MTrk\"";
      if (m_commentsQ)
      {   
